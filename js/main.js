...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function stopSyncing(msg) {
    ...
    grids.forEach((id) => {
      ...
      if (
        el &&
        (el.innerText.includes("SYNCING") || el.innerText.includes("LOADING"))
      ) {
  ...
  function renderUI() {
    if (document.getElementById("tshirts-grid"))
      renderCategory("Tees", "tshirts-grid", 3);
    if (document.getElementById("hoodies-grid"))
      renderCategory("Hoodies", "hoodies-grid", 3);
    if (document.getElementById("tshirts-grid-shop"))
      renderCategory("Tees", "tshirts-grid-shop");
    if (document.getElementById("hoodies-grid-shop"))
      renderCategory("Hoodies", "hoodies-grid-shop");
    if (document.getElementById("shop-grid")) renderShop(allProducts);
    if (document.getElementById("all-products-grid"))
      renderShop(allProducts, "all-products-grid");
  ...
  function createCard(p) {
    const img =
      (p.images && p.images[0]) || "https://via.placeholder.com/600x800";
    return `
  ...
  function renderFeatured() {
    ...
    grid.innerHTML = items.length
      ...
      : '<p style="padding:40px; text-align:center;">Catalog is empty.</p>';
  }

  function renderShop(toRender, elementId = "shop-grid") {
    const grid = document.getElementById(elementId);
    if (!grid) return;
    grid.innerHTML = toRender.length
      ? toRender.map((p) => createCard(p)).join("")
      : '<p style="padding:40px; text-align:center;">No items found.</p>';
  }

  function renderCategory(cat, elementId, limit = null) {
    const grid = document.getElementById(elementId);
    if (!grid) return;
    ...
    grid.innerHTML = items.length
      ? items.map((p) => createCard(p)).join("")
      : `<p style="padding:40px; text-align:center; color: #444; font-weight: 800;">No ${cat} available yet.</p>`;
  }
  ...
  function renderDetail() {
    ...
    if (document.getElementById("pd-name"))
      document.getElementById("pd-name").innerText = product.name;
    if (document.getElementById("pd-price"))
      document.getElementById("pd-price").innerText =
        `${parseFloat(product.price || 0).toFixed(2)} MAD`;
    if (document.getElementById("pd-desc"))
      document.getElementById("pd-desc").innerText = product.description;

    // Safely parse images if they come as a JSON string
    let imgs = [];
    try {
    ...
    if (!imgs || imgs.length === 0 || !imgs[0] || imgs[0] === 'undefined' || imgs[0] === 'null') {
      imgs = ["https://via.placeholder.com/600x800"];
    }
    
    // Filter out empty invalid strings from array
    imgs = imgs.filter(img => typeof img === 'string' && img.trim().length > 0 && img !== 'undefined' && img !== 'null');
    ...
    if (main) {
      main.innerHTML = `<img id="main-product-image" src="${imgs[0]}" alt="${product.name}" style="width:100%; height:100%; object-fit:cover; transition: opacity 0.2s ease;">`;
    }

    const thumbs = document.getElementById("pd-thumbs");
    if (thumbs) {
        if (imgs.length > 1) {
            thumbs.style.marginTop = "20px";
            thumbs.style.display = "flex";
            thumbs.style.flexWrap = "wrap";
            thumbs.style.gap = "10px";
            thumbs.innerHTML = imgs.map((img, idx) => `
                <img src="${img}" 
                     style="width: 80px; height: 100px; object-fit: cover; cursor: pointer; border: 2px solid ${idx === 0 ? '#000' : 'transparent'}; border-radius: 4px;"
                     onclick="
                        document.getElementById('main-product-image').src = this.src;
                        Array.from(this.parentElement.children).forEach(c => c.style.borderColor = 'transparent');
  ...
  window.addToCartFromPD = () => {
    ...
    const selectedSizeEl = Array.from(
      document.querySelectorAll("#pd-sizes div"),
    ).find(
      (el) =>
  ...
  function renderCart() {
    ...
    if (window.cart.length === 0) {
      itemsList.innerHTML =
        '<p style="text-align:center; padding: 40px 0; color: #888;">Votre panier est vide.</p>';
      updateTotals();
      return;
    }

    itemsList.innerHTML = window.cart
      .map(
        (item, index) => `
            <div class="cart-item">
